...
export const calendar = new Hono()
    ...
    try {
        ...
        await db.delete(notification).where(eq(notification.id, data.id));
        const date = await db.select({ month: sql `MONTH(CONVERT_TZ(Event.date, '+00:00', ${c.var.timezone}))`, year: sql `YEAR(CONVERT_TZ(Event.date, '+00:00', ${c.var.timezone}))` }).from(Event).where(eq(Event.id, data.eventId)).then((r) => r[0]);
