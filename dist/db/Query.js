...
export const getUserCalendarId = async (userId) => {
...
export const userHasEvent = async (userId, eventId, action) => {
    let res = await db.select().from(Event).where(and(eq(Event.id, eventId), eq(Event.createBy, userId))).then((r) => r.length == 1);
    if (!res) {
